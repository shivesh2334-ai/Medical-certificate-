...
with st.sidebar:
    ...
    doctor_reg_no = st.text_input("Medical Registration No.*", value="")
...
with tab3:
    ...
    with col2:
        ...
        follow_up = st.text_input("Follow-up Date (if applicable)", key="followup_sl")
...
def generate_medical_certificate(clinic_name, clinic_address, clinic_phone, clinic_email, clinic_reg,
    ...
    pdf.multi_cell(0, 6, f"This is to certify that I, {doctor_name}, {doctor_qualification}, Registration No: {doctor_reg_no}, "
    ...
    filename = f"certificates/Medical_Certificate_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
    pdf.output(filename)
    return filename
...
def generate_fitness_certificate(clinic_name, clinic_address, clinic_phone, clinic_email, clinic_reg,
    ...
    pdf.multi_cell(0, 6, f"This is to certify that I, {doctor_name}, {doctor_qualification}, "
                        f"Registration No: {doctor_reg_no}, have carefully examined "
    ...
    filename = f"certificates/Fitness_Certificate_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
    pdf.output(filename)
    return filename
...
def generate_sick_leave_certificate(clinic_name, clinic_address, clinic_phone, clinic_email, clinic_reg,
    ...
    filename = f"certificates/Sick_Leave_Certificate_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
    pdf.output(filename)
    return filename
...
def generate_form_1a(clinic_name, clinic_address, clinic_phone, clinic_email, clinic_reg,
    ...
    if fit_to_drive:
        pdf.multi_cell(0, 6, f"I, {doctor_name}, {doctor_qualification}, Registration No: {doctor_reg_no}, "
    ...
    filename = f"certificates/Form_1A_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
    pdf.output(filename)
    return filename
